...
class P2Closure(ASTVisitor, stage.Stage):
    # Private Attributes: ###################################################################################
    ...
    def _createMainFunc(self, fun_list, main_ast):
        mainFunc = Function(None, Name('main'), [], [], 0, None, Stmt(main_ast.node.nodes + [Return(Const(0))]))
    ...
    # Public Methods: #######################################################################################
    def do(self):
        (main_ast, fun_list) = self.visit(self._stageInput)
