...
class Myx86Selector:
    ...
    def generate_x86_code(self, ast):
        ...
        if isinstance(ast, Module):
            ...
            if getattr(ast, "tailCall", False) and len(ast.expr.args) == ast.callerArgNum:
                #callFuncNode = ast.expr

                #The commend out code below offsets the stack frame if the caller
                #has a different number of arguments than the callee. Doing this is
                #complicated, and the code below doesn't work. I'll have to return
                #to this later.
                #save eip and ebp and restore to correct place if necessary.
                #newFrameOffset = (len(ast.expr.args) - ast.callerArgNum) * 4
                #newFrameOffset = (ast.callerArgNum - len(ast.expr.args)) * 4
                #if newFrameOffset != 0:
                   # saved_ebp = self.makeTmpVar() #These are guaranteed to be registers because
                   # saved_eip = self.makeTmpVar() #what's moved into them are mem locations
                   # myIRList.append(x86.Movl(x86.MemLoc(0), saved_ebp)) #save previous ebp
                   # myIRList.append(x86.Movl(x86.MemLoc(4), saved_eip)) #save previous eip
                   # myIRList.append(x86.Movl(saved_ebp, x86.MemLoc(newFrameOffset)))    
                   # myIRList.append(x86.Movl(saved_eip, x86.MemLoc(newFrameOffset+4)))
